...
export async function GET() {
  try {
    ...
    if (!session || session.role !== 'KECAMATAN') {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    if (!session.districtId) {
       return NextResponse.json({ error: 'District ID missing from session' }, { status: 400 });
    }
    ...
    villageProgress.sort((a, b) => a.name.localeCompare(b.name));

    return NextResponse.json({
      success: true,
      stats: { notInputted, inputted, notVerified, verified, totalVillages: villages.length },
      villageProgress
    });

  } catch (error) {
    console.error('Kecamatan Dashboard Error:', error);
